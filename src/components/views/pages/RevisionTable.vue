    ...
    <table class="w-full">
      ...
      <tbody>
        <tr v-for="(revision, ndx) in sortedRevisions" :key="revision.id" :class="ndx % 2 ? 'bg-slate-200' : ''">
          ...
          <td class="py-2 px-6">
            <button class="icon-link tooltip" data-tooltip="Ansicht" @click="emit('load-revision', revision)"><eye-icon class="w-5 h-5" /></button>
          </td>
          ...
          <td class="py-2 px-6 text-right">
            <button class="icon-link tooltip" data-tooltip="Löschen" @click="emit('delete-revision', revision)" v-if="!revision.active"><trash-icon class="w-5 h-5" /></button>
          </td>
        </tr>
      </tbody>
    </table>
